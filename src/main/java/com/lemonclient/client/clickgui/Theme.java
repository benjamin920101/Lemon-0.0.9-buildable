...
public class Theme extends ThemeBase {
   ...
   protected void renderSmallButton(Context context, String title, int symbol, boolean focus) {
      ...
      Rectangle rect = new Rectangle(
         context.getPos().x + padding / 2,
         context.getPos().y + padding / 2,
         context.getSize().height - 2 * (padding / 2),
         context.getSize().height - 2 * (padding / 2)
      );
      if (title == null) {
         rect.x = rect.x + (context.getSize().width / 2 - context.getSize().height / 2);
      }
      ...
      if (title != null) {
         int var10004 = symbol == 0 ? padding : context.getSize().height;
         context.getInterface()
            .drawString(new Point(context.getPos().x + var10004, context.getPos().y + padding), this.height, title, this.getFontColor(focus));
      }
   }
   ...
   public <T> IScrollBarRenderer<T> getScrollBarRenderer(Class<T> type, int logicalLevel, final int graphicalLevel) {
      return new IScrollBarRenderer<T>() {
         ...
         public int renderScrollBar(Context context, boolean focus, T state, boolean horizontal, int height, int position) {
            ...
            if (horizontal) {
               int a = (int)((double)position / height * context.getSize().width);
               int b = (int)((double)(position + context.getSize().width) / height * context.getSize().width);
               ...
               int a = (int)((double)position / height * context.getSize().height);
               int b = (int)((double)(position + context.getSize().height) / height * context.getSize().height);
            ...
            return horizontal
               ? (int)((double)((context.getInterface().getMouse().x - context.getPos().x) * height) / context.getSize().width - context.getSize().width / 2.0)
               : (int)(
                  (double)((context.getInterface().getMouse().y - context.getPos().y) * height) / context.getSize().height - context.getSize().height / 2.0
               );
         }
      ...
      return new IButtonRenderer<T>() {
         ...
         public void renderButton(Context context, String title, boolean focus, T state) {
            ...
            if (type == String.class) {
               context.getInterface()
                  .drawString(
                     new Point(context.getPos().x + Theme.this.padding, context.getPos().y + Theme.this.padding),
                     Theme.this.height,
                     title + Theme.this.separator + state,
                     color
                  );
            } else {
               context.getInterface()
                  .drawString(new Point(context.getPos().x + Theme.this.padding, context.getPos().y + Theme.this.padding), Theme.this.height, title, color);
            }
         }
         ...
         public void renderButton(Context context, String title, boolean focus, String state) {
            ...
            context.getInterface()
               .drawString(
                  new Point(context.getPos().x + Theme.this.padding, context.getPos().y + Theme.this.padding),
                  Theme.this.height,
                  title + Theme.this.separator + (focus ? "..." : state),
                  color
               );
         }
      ...
      return new IRadioRenderer() {
         ...
         public void renderItem(Context context, ILabeled[] items, boolean focus, int target, double state, boolean horizontal) {
            ...
            for (int i = 0; i < items.length; i++) {
               ...
               context.getInterface()
                  .drawString(
                     new Point(rect.x + Theme.this.padding, rect.y + Theme.this.padding),
                     Theme.this.height,
                     items[i].getDisplayName(),
                     i == target ? Theme.this.getMainColor(focus, true) : Theme.this.getFontColor(focus)
                  );
            }
         }
   ...
   public IResizeBorderRenderer getResizeRenderer() {
      return new IResizeBorderRenderer() {
         ...
         public void drawBorder(Context context, boolean focus) {
            ...
            context.getInterface()
               .fillRect(new Rectangle(rect.x, rect.y + rect.height - this.getBorder(), rect.width, this.getBorder()), color, color, color, color);
            context.getInterface()
               .fillRect(new Rectangle(rect.x, rect.y + this.getBorder(), this.getBorder(), rect.height - 2 * this.getBorder()), color, color, color, color);
            context.getInterface()
               .fillRect(
                  new Rectangle(rect.x + rect.width - this.getBorder(), rect.y + this.getBorder(), this.getBorder(), rect.height - 2 * this.getBorder()),
                  color,
                  color,
                  color,
                  color
   ...
   public ITextFieldRenderer getTextRenderer(boolean embed, int logicalLevel, final int graphicalLevel, final boolean container) {
      return new ITextFieldRenderer() {
         ...
         public int renderTextField(Context context, String title, boolean focus, String content, int position, int select, int boxPosition, boolean insertMode) {
            ...
            if (boxPosition < position) {
               ...
               while (
                  minPosition < position
                     && context.getInterface().getFontWidth(Theme.this.height, content.substring(0, minPosition)) + rect.width - Theme.this.padding < strlen
               ) {
                  minPosition++;
               }
            ...
            context.getInterface()
               .drawString(
                  new Point(context.getPos().x + Theme.this.padding, context.getPos().y + Theme.this.padding / 2),
                  Theme.this.height,
                  title + Theme.this.separator,
                  textColor
               );
            ...
            if (select >= 0) {
               ...
               context.getInterface()
                  .fillRect(
                     new Rectangle(Math.min(x1, x3), rect.y + Theme.this.padding / 2, Math.abs(x3 - x1), Theme.this.height),
                     highlightColor,
                     highlightColor,
                     highlightColor,
                     highlightColor
            ...
            context.getInterface()
               .drawString(new Point(rect.x + Theme.this.padding / 2 - offset, rect.y + Theme.this.padding / 2), Theme.this.height, content, textColor);
            if (System.currentTimeMillis() / 500L % 2L == 0L && focus) {
               if (insertMode) {
                  context.getInterface()
                     .fillRect(new Rectangle(x1, rect.y + Theme.this.padding / 2 + Theme.this.height, x2 - x1, 1), textColor, textColor, textColor, textColor);
               } else {
                  context.getInterface()
                     .fillRect(new Rectangle(x1, rect.y + Theme.this.padding / 2, 1, Theme.this.height), textColor, textColor, textColor, textColor);
               }
            }
      ...
      return new ISwitchRenderer<Boolean>() {
         public void renderButton(Context context, String title, boolean focus, Boolean state) {
            ...
            context.getInterface()
               .drawString(
                  new Point(context.getPos().x + Theme.this.padding, context.getPos().y + Theme.this.padding),
                  Theme.this.height,
                  title + Theme.this.separator + (state ? "On" : "Off"),
                  Theme.this.getFontColor(focus)
               );
            ...
            rect = new Rectangle(
               rect.x + rect.width - 2 * rect.height + 3 * Theme.this.padding,
               rect.y + Theme.this.padding,
               2 * rect.height - 4 * Theme.this.padding,
               rect.height - 2 * Theme.this.padding
            );
         ...
         public Rectangle getOnField(Context context) {
            ...
            return new Rectangle(
               rect.x + rect.width - rect.height + Theme.this.padding,
               rect.y + Theme.this.padding,
               rect.height - 2 * Theme.this.padding,
               rect.height - 2 * Theme.this.padding
            );
