...
public class HoleSnap extends Module {
   ...
   private final Listener<PlayerMoveEvent> playerMoveListener = new Listener<>(
      event -> {
         ...
         if (++this.enabledTicks > this.timeoutTicks.getValue()) {
            this.disable();
         } else if (mc.player.isEntityAlive() && !mc.player.isElytraFlying() && !mc.player.capabilities.isFlying) {
            ...
            if (this.shouldDisable(currentSpeed)) {
               ...
               if (hole != null) {
                  ...
                  if (this.checkYRange((int)mc.player.posY, this.originPos.y)
                     && mc.player.getDistance(x + 0.5, mc.player.posY, z + 0.5) <= this.hRange.getValue()) {
                     this.isActive = true;
   ...
   public static EntityPlayer getNearestPlayer() {
      return mc.world
         .playerEntities
         .stream()
         .filter(p -> mc.player.getDistance(p) <= 8.0F)
         .filter(p -> mc.player.entityId != p.entityId)
         .filter(p -> !EntityUtil.basicChecksEntity(p))
         .filter(p -> HoleUtil.isInHole(p, false, true, false))
   ...
   private BlockPos findHoles() {
      ...
      boolean near = this.near.getValue() && target != null;
      NonNullList<HoleSnap.HoleBlock> holes = NonNullList.create();
      List<BlockPos> blockPosList = EntityUtil.getSphere(PlayerUtil.getPlayerPos(), this.hRange.getValue(), 8.0, false, true, 0);
               ...
               if (mc.world.isAirBlock(PlayerUtil.getPlayerPos().up(2)) || (int)mc.player.posY >= pos.y) {
                  ...
                  if (holeType != HoleUtil.HoleType.NONE) {
                     ...
                     if (mc.world.isAirBlock(pos)
                        ...
                        holes.add(
                           new HoleSnap.HoleBlock(
                              pos,
                              (
                                    near
                                       ? target.getDistance(pos.x + 0.5, pos.y, pos.z + 0.5)
                                       : mc.player.getDistance(pos.x + 0.5, pos.y, pos.z + 0.5)
                                 )
                                 + (this.bedrock.getValue() ? (holeInfo.getSafety() == HoleUtil.BlockSafety.UNBREAKABLE ? -100 : 0) : 0)
                           )
                        );
                     }
                  }
      ...
      return holes.isEmpty() ? null : holes.stream().min(Comparator.comparing(p -> p.value)).orElse(null).pos;
   }
