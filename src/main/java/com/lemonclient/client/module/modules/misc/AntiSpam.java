...
public class AntiSpam extends Module {
   ...
   IntegerSetting duplicatesTimeout = this.registerInteger("Duplicates Timeout", 30, 1, 600, () -> this.duplicates.getValue());
   ...
   IntegerSetting ignoreDuration = this.registerInteger("Ignore Duration", 120, 0, 43200, () -> this.autoIgnore.getValue());
   IntegerSetting violations = this.registerInteger("Violations", 3, 1, 100, () -> this.autoIgnore.getValue());
         ...
         if (mc.player != null && this.isEnabled()) {
            if (event.getPacket() instanceof SPacketChat) {
               ...
               if (!username.equals("null")
                  && mc.player.connection.getPlayerInfo(username) != null
                  && !SocialManager.isIgnore(username)
                  && (!this.filterFriend.getValue() || !SocialManager.isOnFriendList(username))) {
                  SPacketChat sPacketChat = (SPacketChat)event.getPacket();
                  if (this.detectSpam(sPacketChat.getChatComponent().getUnformattedText()) && !username.equalsIgnoreCase(mc.player.getName())) {
                     if (this.autoIgnore.getValue()) {
                        if ((this.violate.get(username) == null || this.violate.get(username) < this.violations.getValue()) && this.violations.getValue() != 0) {
                           ...
                           MessageBus.sendMessage(
                              ChatFormatting.RED + username + " has exceeded the limitation of spam violation, ignoring.",
   ...
   public void startIgnore(final String finalUsername) {
      if (!mc.player.getName().equalsIgnoreCase(finalUsername)) {
         if (!SocialManager.isIgnore(finalUsername)) {
            ...
            MessageBus.sendMessage(
               ChatFormatting.RED
                  + finalUsername
                  + " has been auto ignored by AntiSpam for "
                  + this.ignoreDuration.getValue()
                  + (this.ignoreDuration.getValue() > 1 ? " seconds" : " second"),
               Notification.Type.INFO,
               "AntiSpam",
               13,
               false
   ...
   private boolean detectSpam(String message) {
      if (this.greenText.getValue() && this.findPatterns(AntiSpam.FilterPatterns.GREEN_TEXT, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] Green Text: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.discordLinks.getValue() && this.findPatterns(AntiSpam.FilterPatterns.DISCORD, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] Discord Link: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.webLinks.getValue() && this.findPatterns(AntiSpam.FilterPatterns.WEB_LINK, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] Web Link: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.ips.getValue() && this.findPatterns(AntiSpam.FilterPatterns.IP_ADDR, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] IP Address: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.ipsAgr.getValue() && this.findPatterns(AntiSpam.FilterPatterns.IP_ADDR_AGR, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] IP Aggressive: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.tradeChat.getValue() && this.findPatterns(AntiSpam.FilterPatterns.TRADE_CHAT, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] Trade Chat: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.numberSuffix.getValue() && this.findPatterns(AntiSpam.FilterPatterns.NUMBER_SUFFIX, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] Number Suffix: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.announcers.getValue() && this.findPatterns(AntiSpam.FilterPatterns.ANNOUNCER, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] Announcer: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.spammers.getValue() && this.findPatterns(AntiSpam.FilterPatterns.SPAMMER, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] Spammers: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.insulter.getValue() && this.findPatterns(AntiSpam.FilterPatterns.INSULTER, message)) {
         if (this.showBlocked.getValue()) {
            MessageBus.sendMessage("[AntiSpam] Insulter: " + message, Notification.Type.INFO, "AntiSpam", 13, false);
         }

         return true;
      } else if (this.greeters.getValue() && this.findPatterns(AntiSpam.FilterPatterns.GREETER, message)) {
         if (this.showBlocked.getValue()) {
         ...
         if (this.duplicates.getValue()) {
            ...
            boolean isDuplicate = this.messageHistory.containsKey(message)
               && (System.currentTimeMillis() - this.messageHistory.get(message)) / 1000L < this.duplicatesTimeout.getValue().intValue();
   ...
   private static class FilterPatterns {
      private static final String[] ANNOUNCER = new String[]{
         "I just walked .+ feet!",
         "I just placed a .+!",
      ...
      private static final String[] SPAMMER = new String[]{
      ...
      private static final String[] INSULTER = new String[]{
      ...
      private static final String[] IP_ADDR = new String[]{
