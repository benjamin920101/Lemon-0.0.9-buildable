...
public class AutoRespawn extends Module {
      ...
      if (this.isEnabled()) {
         if (event.getGui() instanceof GuiGameOver) {
            ...
            if (this.respawn.getValue()) {
               ...
               mc.player.connection.sendPacket(new CPacketClientStatus(State.PERFORM_RESPAWN));
            }
         }
      }
   });
   ...
   public void onUpdate() {
      if (mc.player != null) {
         if (this.isDead && mc.player.isEntityAlive()) {
            if (this.coords.getValue()) {
               MessageBus.sendMessage(
                  "You died at X:" + this.deathPos.getX() + ", Y:" + this.deathPos.getY() + ", Z:" + this.deathPos.getZ() + ".",
                  false
               );
            }
         ...
         if (!this.sentRespawnMessage && System.currentTimeMillis() - this.timeSinceRespawn > this.respawnMessageDelay.getValue().intValue()) {
            mc.player.connection.sendPacket(new CPacketChatMessage(this.message.getText()));
            this.sentRespawnMessage = true;
         }
      }
   }
