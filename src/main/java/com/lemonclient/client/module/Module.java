...
public abstract class Module implements Listenable {
   ...
   private Module.Declaration getDeclaration() {
      return this.getClass().getAnnotation(Module.Declaration.class);
   }
   ...
   public void enable() {
      ...
      try {
         ...
         for (StackTraceElement stack : var6.getStackTrace()) {
            System.out.println(stack.toString());
         }
      }

      if (this.toggleMsg && mc.world != null && mc.player != null) {
         MessageBus.sendClientDeleteMessage(
            ModuleManager.getModule(ColorMain.class).getModuleColor()
               + this.name
               + ChatFormatting.GRAY
               + " turned "
