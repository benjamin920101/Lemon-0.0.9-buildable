...
public class RotationUtil {
   ...
   public static float[] getRotations(BlockPos pos, EnumFacing facing, Entity from, World world, IBlockState state) {
      AxisAlignedBB bb = state.getSelectedBoundingBox(world, pos);
   ...
   public static EntityPlayer getRotationPlayer() {
      ...
      return (EntityPlayer)(rotationEntity == null ? mc.player : rotationEntity);
   }
   ...
   public static boolean isInFov(BlockPos pos) {
      return pos != null && (mc.player.getDistanceSq(pos) < 4.0 || isInFov(new Vec3d(pos), mc.player.getPositionVector()));
   }
   ...
   public static float transformYaw() {
      ...
      if (mc.player.rotationYaw > 0.0F && yaw > 180.0F) {
   ...
   public static float[] getRotationsBlock(BlockPos block, EnumFacing face, boolean Legit) {
      double x = block.getX() + 0.5 - mc.player.posX + face.getXOffset() / 2.0;
      double z = block.getZ() + 0.5 - mc.player.posZ + face.getZOffset() / 2.0;
