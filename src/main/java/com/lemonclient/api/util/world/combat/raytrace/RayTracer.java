...
public class RayTracer {
   ...
   public static RayTraceResult rayTraceEntities(
      World world, Entity from, double range, EntityPlayer player, Predicate<Entity> entityCheck, Entity... additional
   ) {
      Vec2f vec = PlayerPacketManager.INSTANCE.getServerSideRotation();
   ...
   public static RayTraceResult rayTraceEntities(
      ...
      Vec3d eyePos = new Vec3d(posX, posY + from.getEyeHeight(), posZ);
      ...
      if (mc.isCallingFromMinecraftThread()) {
         ...
         entities = mc.world
            .loadedEntityList
            .stream()
            .filter(e -> e != null && e.getEntityBoundingBox().intersects(within) && predicate.test(e))
      ...
      for (Entity entityx : entities) {
         AxisAlignedBB bb = entityx.getEntityBoundingBox().grow(entityx.getCollisionBorderSize());
         RayTraceResult result = bb.calculateIntercept(eyePos, intercept);
         if (bb.contains(eyePos)) {
            if (distance >= 0.0) {
               pointedEntity = entityx;
            ...
            if (hitDistance < distance || distance == 0.0) {
               if (entityx.getLowestRidingEntity() == from.getLowestRidingEntity()) {
                  if (distance == 0.0) {
                     pointedEntity = entityx;
      ...
      return pointedEntity != null && hitVec != null ? new RayTraceResult(pointedEntity, hitVec) : null;
   }

   public static RayTraceResult trace(
      World world,
      IBlockAccess access,
      Vec3d start,
      Vec3d end,
   ...
   public static RayTraceResult trace(
      ...
      return traceTri(
         ...
         blockChecker == null ? null : (b, p, ef) -> blockChecker.test(b, p)
      );
   }

   public static RayTraceResult traceTri(
      World world,
      IBlockAccess access,
      Vec3d start,
      Vec3d end,
   ...
   public static RayTraceResult traceTri(
      ...
      if (Double.isNaN(start.x) || Double.isNaN(start.y) || Double.isNaN(start.z)) {
         ...
         if ((!ignoreBlockWithoutBoundingBox || state.getCollisionBoundingBox(access, pos) != Block.NULL_AABB)
            && (block.canCollideCheck(state, stopOnLiquid) || collideCheck != null && collideCheck.test(block, pos, null))
            && (blockChecker == null || blockChecker.test(block, pos, null))) {
            RayTraceResult raytraceresult = crt.collisionRayTrace(state, world, pos, start, end);
         ...
         while (steps-- >= 0) {
            ...
            if (!ignoreBlockWithoutBoundingBox
               ...
               || state1.getCollisionBoundingBox(access, pos) != Block.NULL_AABB) {
               if ((block1.canCollideCheck(state1, stopOnLiquid) || collideCheck != null && collideCheck.test(block1, pos, enumfacing))
                  && (blockChecker == null || blockChecker.test(block1, pos, enumfacing))) {
                  RayTraceResult raytraceresult1 = crt.collisionRayTrace(state1, world, pos, start, end);
                  if (raytraceresult1 != null) {
                     return raytraceresult1;
                  }
               } else {
                  result = new RayTraceResult(Type.MISS, start, enumfacing, pos);
               }
            }
         }
