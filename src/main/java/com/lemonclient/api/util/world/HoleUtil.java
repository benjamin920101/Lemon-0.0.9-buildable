...
public class HoleUtil {
   ...
   public static boolean isInHole(Entity e, boolean onlyOneWide, boolean ignoreDown, boolean render) {
      return !isHole(new BlockPos(e.getPositionVector()), onlyOneWide, ignoreDown, render).getType().equals(HoleUtil.HoleType.NONE);
   }

   public static boolean isHoleBlock(BlockPos blockPos, boolean onlyOneWide, boolean ignoreDown, boolean render) {
      return !isHole(blockPos, onlyOneWide, ignoreDown, render).getType().equals(HoleUtil.HoleType.NONE)
         && !isHole(blockPos, onlyOneWide, ignoreDown, render).getType().equals(HoleUtil.HoleType.CUSTOM);
   }
   ...
   public static HoleUtil.BlockSafety isBlockSafe(Block block) {
      if (block == Blocks.BEDROCK) {
         ...
         return block != Blocks.OBSIDIAN && block != Blocks.ENDER_CHEST && block != Blocks.ANVIL
            ? HoleUtil.BlockSafety.BREAKABLE
            : HoleUtil.BlockSafety.RESISTANT;
      }
   }
   ...
   private static HoleUtil.HoleInfo isFourHole(HoleUtil.HoleInfo info, BlockPos centreBlock, boolean render) {
      if (render ? HoleFinder.is2x2single(centreBlock, true) : HoleFinder.is2x2(centreBlock, true)) {
         info.setSafety(HoleUtil.BlockSafety.RESISTANT);
         ...
         if (!render) {
            ...
            if (mc.world.isAirBlock(centreBlock.add(-1, 0, 0))) {
               blockPos = centreBlock.add(-1, 0, 0);
            }

            if (mc.world.isAirBlock(centreBlock.add(0, 0, -1))) {
   ...
   public static HashMap<HoleUtil.BlockOffset, HoleUtil.BlockSafety> getUnsafeSides(BlockPos pos) {
      ...
      HoleUtil.BlockSafety temp = isBlockSafe(mc.world.getBlockState(HoleUtil.BlockOffset.DOWN.offset(pos)).getBlock());
