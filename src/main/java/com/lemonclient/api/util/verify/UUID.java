...
public class UUID {
   ...
   private static final UUID.ConnectionFactory CONNECTION_FACTORY = UUID.ConnectionFactory.DEFAULT;
   ...
   private UUID.UploadProgress progress = UUID.UploadProgress.DEFAULT;
   ...
   protected UUID copy(final InputStream input, final OutputStream output) throws IOException {
      return (new UUID.CloseOperation<UUID>(input, this.ignoreCloseExceptions) {
         public UUID run() throws IOException {
            ...
            while ((read = input.read(buffer)) != -1) {
               ...
               UUID.this.totalWritten = UUID.this.totalWritten + read;
