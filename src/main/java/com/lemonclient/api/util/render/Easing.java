...
public interface Easing {
   ...
   public static class BackIn extends Easing.Back {
      ...
      public float ease(float time, float startTime, float change, float endTime) {
         ...
         float var6;
         return change * (var6 = time / endTime) * var6 * ((s + 1.0F) * var6 - s) + startTime;
      }
   }
   ...
   public static class BackInOut extends Easing.Back {
      ...
      public float ease(float time, float startTime, float change, float endTime) {
         ...
         float var8;
         float var9;
         return (var6 = time / (endTime / 2.0F)) < 1.0F
            ? change / 2.0F * (var6 * var6 * (((var8 = (float)(s * 1.525)) + 1.0F) * var6 - var8)) + startTime
            : change / 2.0F * ((var7 = var6 - 2.0F) * var7 * (((var9 = (float)(s * 1.525)) + 1.0F) * var7 + var9) + 2.0F) + startTime;
      }
   }
   ...
   public static class BackOut extends Easing.Back {
      ...
      public float ease(float time, float startTime, float change, float endTime) {
         float s = this.getOvershoot();
         float var6;
         return change * ((var6 = time / endTime - 1.0F) * var6 * ((s + 1.0F) * var6 + s) + 1.0F) + startTime;
      }
   }
   ...
   public static class ElasticIn extends Easing.Elastic {
      ...
      public float ease(float time, float startTime, float change, float endTime) {
         ...
         if (time == 0.0F) {
            return startTime;
         } else if ((time = time / endTime) == 1.0F) {
            ...
            return -(a * (float)Math.pow(2.0, 10.0F * --time) * (float)Math.sin((time * endTime - s) * (Math.PI * 2) / p)) + startTime;
         }
      }
   }
   ...
   public static class ElasticInOut extends Easing.Elastic {
      ...
      public float ease(float time, float startTime, float change, float endTime) {
         ...
         if (time == 0.0F) {
            return startTime;
         } else if ((time = time / (endTime / 2.0F)) == 2.0F) {
            return startTime + change;
         } else {
            if (p == 0.0F) {
            ...
            return time < 1.0F
               ? -0.5F * (a * (float)Math.pow(2.0, 10.0F * --time) * (float)Math.sin((time * endTime - s) * (Math.PI * 2) / p)) + startTime
               : a * (float)Math.pow(2.0, -10.0F * --time) * (float)Math.sin((time * endTime - s) * (Math.PI * 2) / p) * 0.5F + change + startTime;
         }
      }
   }
   ...
   public static class ElasticOut extends Easing.Elastic {
      ...
      public float ease(float time, float startTime, float change, float endTime) {
         ...
         if (time == 0.0F) {
            ...
            return a * (float)Math.pow(2.0, -10.0F * time) * (float)Math.sin((time * endTime - s) * (Math.PI * 2) / p) + change + startTime;
         }
      }
   }
}
