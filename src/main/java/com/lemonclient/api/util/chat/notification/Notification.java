...
public abstract class Notification {
   ...
   protected int getOffset(double maxWidth) {
      if (this.getTime() < this.fadedIn) {
         return (int)(Math.tanh((double)this.getTime() / this.fadedIn * 3.0) * maxWidth);
      } else {
         return this.getTime() > this.fadeOut
            ? (int)(Math.tanh(3.0 - (double)(this.getTime() - this.fadeOut) / (this.end - this.fadeOut) * 3.0) * maxWidth)
            : (int)maxWidth;
      }
   }
